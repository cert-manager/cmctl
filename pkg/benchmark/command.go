...
func NewCmd(ctx context.Context, ioStreams genericiooptions.IOStreams) *cobra.Command {
	...
	cmd.Flags().Int64Var(&options.rampUpTargetCertificateCount, "phases.ramp-up.target-certificate-count", 2000,
		"The number of Certificate resources to create during the ramp-up phase.")

	cmd.Flags().DurationVar(&options.steadyStateDuration, "phases.steady-state.duration", time.Minute*10,
		"The duration of the steady-state phase.")

	cmd.Flags().DurationVar(&options.finalMeasurementsDuration, "phases.final-measurements.duration", time.Minute*2,
