...
import (
	...
	"github.com/cert-manager/ctl/pkg/upgrade/migrateapiversion"
	"github.com/cert-manager/ctl/test/integration/framework"
)
...
func crdFolder(t *testing.T) string {
	val, ok := os.LookupEnv("TEST_CRDS")
	if !ok {
		t.Fatal("TEST_CRDS environment variable must be set")
	}
	return val
}
